...
<?php
		if (isset($_GET['rentor_ic']))
			$rentorIC = $_GET['rentor_ic'];
		else
			$rentorIC = 0;
		...
		$query = "SELECT `rent`.*,`rentor`.*,`asset`.* FROM `rent`
		LEFT JOIN `rentor` ON `rentor`.`rentor_ic` = `rent`.`rentor_ic`
		RIGHT JOIN `asset` ON `asset`.`asset_id` = `rent`.`asset_id`
		WHERE `rent`.`rentor_ic` =  '$rentorIC'";
	
		$query1 = "SELECT `rent`.*,`rentor`.*,`asset`.* FROM `rent`
		LEFT JOIN `rentor` ON `rentor`.`rentor_ic` = `rent`.`rentor_ic`
		RIGHT JOIN `asset` ON `asset`.`asset_id` = `rent`.`asset_id`
		WHERE `rent`.`rentor_ic` = '$rentorIC'";
